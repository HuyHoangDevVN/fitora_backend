...
public class UserGrpcService : UserService.UserServiceBase
{
    ...
    // {
    //     var userRequest = new global::UserService.Application.DTOs.User.Requests.GetUserRequest(
    ...
    // {
    //     var usersRequest = new global::UserService.Application.DTOs.User.Requests.GetUsersRequest(
    //         Username: request.Username ?? string.Empty,
    //         Email: request.Email ?? string.Empty
    //     )
    //     {
    ...
    public override async Task<GetUserInfoBatchResponse> GetUserInfoBatch(GetUserInfoBatchRequest request,
        ServerCallContext context)
    {
        ...
        var users = await _userRepository.GetUsersByIdsAsync(ConvertByteStringToGuid(request.Id), userIds);

        var response = new GetUserInfoBatchResponse();
        response.Users.AddRange(users.Select(u => new GetUserInfoResponse
        {
            Id = ByteString.CopyFrom(u.Id.ToByteArray()),
            IsFriend = u.IsFriend.HasValue ? u.IsFriend.Value : false,  
            IsFollowing = u.IsFollowing.HasValue ? u.IsFollowing.Value : false,  
            Email = u.Email ?? string.Empty,
            Username = u.Username ?? string.Empty,
            FirstName = u.FirstName ?? string.Empty,
            LastName = u.LastName ?? string.Empty,
