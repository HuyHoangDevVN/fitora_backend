...
public class OcrDocumentHandler(IApplicationDbContext db, IAuthorizeExtension auth)
    : ICommandHandler<OcrDocumentCommand, OcrDocumentResult>
{
    public async Task<OcrDocumentResult> Handle(OcrDocumentCommand cmd, CancellationToken ct)
    {
        ...
        return new(v.DocumentNumber, v.FullName, v.DateOfBirth, v.Address, "OCR extracted (mock) - please verify");
    }
}
